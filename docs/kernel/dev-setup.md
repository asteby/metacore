...
the kernel into an application, read [`consumer-guide.md`](./consumer-guide)
instead.
...
> [`consumer-guide.md`](./consumer-guide) and
> [`embedding-quickstart.md`](./embedding-quickstart). Working on the
> dynamic CRUD framework? Read [`dynamic-system.md`](./dynamic-system)
...
The full statement is in [`ARCHITECTURE.md`](https://github.com/asteby/metacore-kernel/blob/main/ARCHITECTURE.md). The four
...
- **Schema-affecting changes touch three places.** Adding a column type
  ...
  and a corresponding entry in [`dynamic-system.md`](./dynamic-system)
...
[`release.md`](./release). In short: `git push origin vX.Y.Z` runs the
