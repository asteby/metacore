...
| `GET/POST/PUT/DELETE /api/dynamic/:model` | [`dynamic/`](https://github.com/asteby/metacore-kernel/blob/main/dynamic/) (auto-mounted) |
| `GET  /api/options/:model`                | [`dynamic/`](https://github.com/asteby/metacore-kernel/blob/main/dynamic/) (host calls `MountOptions` to enable) |
| `GET  /api/search/:model`                 | [`dynamic/`](https://github.com/asteby/metacore-kernel/blob/main/dynamic/) (host calls `MountOptions` to enable) |
...
| `GET  /api/metrics`                       | [`metrics/`](https://github.com/asteby/metacore-kernel/blob/main/metrics/) (mounted on the same router passed to `Mount`) |
...
([`migrations/runner.go`](https://github.com/asteby/metacore-kernel/blob/main/migrations/)) on every boot — idempotent,
...
`host.Host` ([`host/host.go`](https://github.com/asteby/metacore-kernel/blob/main/host/host.go)) owns the `Installer`,
...
See [`dynamic-system.md`](./dynamic-system) for the full installer
...
The full request/response reference is in [`dynamic-api.md`](./dynamic-api).
...
See [`permissions.md`](./permissions) for the complete capability model.
...
Hook the runtime up to your host's base URL and JWT — the SDK
[Consumer Guide](/sdk/consumer-guide) covers the React integration end to
...
- [`dynamic-system.md`](./dynamic-system) — what really happens when an
  addon ships `model_definitions[]`.
- [`dynamic-api.md`](./dynamic-api) — every endpoint, every parameter.
- [`permissions.md`](./permissions) — user gates, addon gates, modes.
- [`consumer-guide.md`](./consumer-guide) — long-form embedding guide.
- [`dev-setup.md`](./dev-setup) — contributing to the kernel itself.
