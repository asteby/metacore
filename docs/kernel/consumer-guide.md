...
> [`embedding-quickstart.md`](./embedding-quickstart). Looking for the dynamic
> CRUD framework spec? See [`dynamic-system.md`](./dynamic-system). For
> permission details, [`permissions.md`](./permissions).
...
`App.RegisterModel(key, factory)` ([`host/app.go`](https://github.com/asteby/metacore-kernel/blob/main/host/app.go)) wires a
...
created — see [`dynamic-system.md`](./dynamic-system) for declarative
...
turn it on. See [`permissions.md`](./permissions) for the full model
...
`Hub.SendToUsers` ([`ws/hub.go`](https://github.com/asteby/metacore-kernel/blob/main/ws/hub.go)) is fire-and-forget,
non-blocking, and per-process. For multi-replica deployments, fan out via
the addon event bus ([`events/`](https://github.com/asteby/metacore-kernel/blob/main/events/)) and have each replica
...
See [`dynamic-system.md`](./dynamic-system), section *Real-time updates*,
