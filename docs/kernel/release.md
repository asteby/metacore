...
5. **Consumer dispatch** — `POST /repos/{owner}/{repo}/dispatches` to every
   consumer host repo with `event_type=metacore-kernel-released`. Each
   consumer can subscribe via `on: repository_dispatch` to run Renovate
   immediately.
...
Renovate, configured per [`consumer-guide.md`](./consumer-guide#8-renovate-template),
