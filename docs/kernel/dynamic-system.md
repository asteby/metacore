...
The complete schema is in [`manifest/manifest.go`](https://github.com/asteby/metacore-kernel/blob/main/manifest/manifest.go).
...
[`dynamic/model.go`](https://github.com/asteby/metacore-kernel/blob/main/dynamic/model.go) for the canonical mapping.
...
`host.App` ([`host/app.go`](https://github.com/asteby/metacore-kernel/blob/main/host/app.go)), every request below is wired
...
`FieldDef` live in [`modelbase/metadata.go`](https://github.com/asteby/metacore-kernel/blob/main/modelbase/metadata.go) — they
...
[Consumer Guide](/sdk/consumer-guide) for advanced rendering
...
in [`permission/middleware.go`](https://github.com/asteby/metacore-kernel/blob/main/permission/middleware.go).
...
[`security/enforcer.go`](https://github.com/asteby/metacore-kernel/blob/main/security/enforcer.go) `ModeFromEnv`). Every
violation also calls `Enforcer.OnViolation` if set — wire it to a Prometheus
counter for an audit feed.

The full reference is in [`permissions.md`](./permissions).

## Real-time updates

The dynamic CRUD layer does **not** automatically broadcast changes to
WebSocket clients. The kernel ships a hub
([`ws/hub.go`](https://github.com/asteby/metacore-kernel/blob/main/ws/hub.go)) and the host's CRUD handler is free to call it,
...
event bus ([`events/`](https://github.com/asteby/metacore-kernel/blob/main/events/)) and have each replica subscribe to its
...
| ------------------------------------------- | ---------------------------------------------------------------- |
| Custom validation (cross-field, async)      | `dynamic.Hooks.BeforeCreate` / `BeforeUpdate` — see [`dynamic/hooks.go`](https://github.com/asteby/metacore-kernel/blob/main/dynamic/hooks.go) |
...
- [`dynamic-api.md`](./dynamic-api) — full HTTP API reference with curl examples.
- [`permissions.md`](./permissions) — capability model, modes, store implementations.
- [`embedding-quickstart.md`](./embedding-quickstart) — your first host with the kernel embedded.
- [`consumer-guide.md`](./consumer-guide) — long-form embedding guide.
- [`../ARCHITECTURE.md`](https://github.com/asteby/metacore-kernel/blob/main/ARCHITECTURE.md) — the four laws of the kernel.
